...
- [ ] **Product Components**: Higher-level, composed product UI — assembled from the primitives above rather than 1:1 Figma atoms.
    ...
    - [x] **Widgets > Finance & Banking**: 16 dashboard widgets (Stock Market Tracker, My Cards, My Cards Vertical, Spending Summary, Exchange, Currency List, Recent Transactions, My Subscriptions, Quick Transfer, Donation Profile, Saved Actions, Total Balance, Total Expenses, Major Expenses, Credit Score, Budget Overview), populated + empty, on the same `WidgetCard` shell. Adds `CreditCard`/`CardChip`/`CountryFlag`/`BrandLogo` helpers; inline SVG charts (line/area/stacked/gauge). `Product Components/Widgets/`.
- [x] **Sector Products**: Full, industry-specific application screens assembled entirely from Product Components (the level above Product Components: Components → Product Components → Sector Products).
    - [x] **HR Management**: A complete HR dashboard — `SidebarNavigation` + a `PageHeader` top bar + the 14 HR widgets in a responsive masonry. `Sector Products/HR Management/`.
        - [x] **Calendar Page Components**: `Calendar` (weekly time-grid with positioned event cards + disabled column), `CalendarCard` (Meeting/Event, 4 sizes, completed), `CalendarCell` (card positioning + empty/disabled), `CalendarUpcoming` (upcoming summary card), `UpcomingStatus` (Later/Cancelled/Conflicted/Today pill).
        - [x] **Navigation Onboarding**: Onboarding top bar with a numbered step indicator + "Need help? Contact Us" + close.
